...
export const Layout: React.FC<LayoutProps> = ({ currentModule, onModuleChange, children }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans">
      {/* Sidebar */}
      <aside className="fixed left-0 top-0 h-screen w-20 lg:w-64 bg-slate-900 text-white flex flex-col z-50 transition-all duration-300">
        <div className="p-6 flex items-center gap-3 border-b border-slate-800">
          ...
          <span className="font-bold text-xl tracking-tight hidden lg:block bg-gradient-to-r from-white to-slate-400 bg-clip-text text-transparent">ENGLDOM</span>
        </div>

        <nav className="flex-1 py-6 px-2 lg:px-4 space-y-1 overflow-y-auto scrollbar-hide">
        ...
        <div className="p-4 border-t border-slate-800 hidden lg:block">
            <button 
                ...
                className="w-full bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-white py-2 px-3 rounded-lg text-xs font-bold flex items-center justify-between transition-all"
            >
